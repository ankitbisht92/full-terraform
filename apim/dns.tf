...
resource "azurerm_dns_ns_record" "this" {
  for_each            = { for ns in var.public_ns_record : ns.public_dns_zone => ns }
  name                = each.value.public_ns_record_name
  zone_name           = each.value.public_dns_zone
  resource_group_name = data.azurerm_resource_group.resource_group.name
  ttl                 = 300
  records             = each.value.public_ns_name_server
  tags                = merge(local.default_tags, var.tags)
}
