...
resource "azurerm_monitor_diagnostic_setting" "diagnostic_setting" {
  ...
  log {
    ...
    retention_policy {
      enabled = true
      days    = var.retention_days.turned_on
    }
  }
  ...
  log {
    ...
    retention_policy {
      days    = var.retention_days.turned_off
      enabled = false
    }
  }
}
...
//audit and metric logging for centralised workspace
resource "azurerm_monitor_diagnostic_setting" "central_diagnostic_setting" {
  count                      = var.centralised_log_analytics_workspace_id != null ? 1 : 0
